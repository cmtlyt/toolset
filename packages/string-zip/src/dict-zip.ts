...
function genKey(keys: string[], length: number) {
  ...
  } while (keys.includes(id) && ++count < total);
  return id;
}
...
function _unzip(zipSource: ZipSource, preKey = '#') {
  ...
  const reg = new RegExp(`{${preKey}(.{${keyLength}})}`, 'g');
  let match: RegExpExecArray | null = reg.exec(source);
  while ((match)) {
...
export function createZip(options: TExclude<CreateWorkerFuncOptions, 'needPost'>) {
  return createWorkerFunc(zipSync, [_zip, getPreKey, genKey, getRandomString], { ...options, needPost: false }).run;
}

export function createUnzip(options: TExclude<CreateWorkerFuncOptions, 'needPost'>) {
  return createWorkerFunc(unzipSync, [_unzip, getPreKey, genKey, getRandomString], { ...options, needPost: false }).run;
}
