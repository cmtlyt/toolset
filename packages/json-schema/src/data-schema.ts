...
function _encodeDataSchema(data: any, map = new Map<any, any>()) {
  ...
  if (staticType.includes(type))
    result = { ...baseSchema, value: data };
  if (type === 'array') {
    result = { ...baseSchema, value: (data as any[]).map(item => _encodeDataSchema(item, map)) };
  }
  else if (type === 'object') {
    result = {
      ...baseSchema,
    ...
    result = {
      ...
      value: (() => {
        ...
        if (funcStr.indexOf('async') === 0)
          return funcStr.replace('async', 'return async function');
    ...
    result = { ...baseSchema, value: data.toISOString() };
  }
  else if (type === 'regexp') {
    result = {
      ...baseSchema,
...
export function encodeDataSchema(data: any) {
  countId = 0;
  let map: Map<any, any> | null = new Map();
...
function _decodeDataSchema(dataSchema: TObject<any>, map: Record<string, any> = {}) {
  ...
  if (staticType.includes(type))
    return dataSchema.value;
  if (map[dataSchema.id])
    return map[dataSchema.id];
  let result: any = 'unknown';
  if (type === 'array') {
    result = (dataSchema.value as any[]).map(item => _decodeDataSchema(item), map);
  }
  else if (type === 'object') {
    result = Object.keys(dataSchema.value).reduce<Record<string, any>>((prev, key) => {
    ...
    result = new Function(dataSchema.value)();
  }
  else if (type === 'date') {
    result = new Date(dataSchema.value);
  }
  else if (type === 'regexp') {
    result = new RegExp(dataSchema.value.source, dataSchema.value.flags);
  }
